...
function EditorPanel({project,file,setFile}){
 ...
 const save=async()=>{if(!file?.path)return;const target=project.files.find(x=>x.path===file.path);if(!target)return;setFile({...file,saved:false,error:'This browser workspace opens repositories read-only. Edit/save is intentionally disabled in M0.'})};
 return <section className="panel editor-panel"><div className="section-head"><div><span className="eyebrow">SOURCE EDITOR</span><h2>{file?.path||'Select a source file'}</h2><p className="muted">Read-only local source inspection with line numbers.</p></div><button disabled={!file} onClick={save}>Save</button></div>{!file?<Empty text="Select a file from Explorer or Search."/>:<div className="editor"><div className="editor-lines">{file.content.split(/\r?\n/).map((_,i)=><span key={i}>{i+1}</span>)}</div><pre className="source editor-source">{file.content}</pre></div>}{file?.error&&<div className="callout">{file.error}</div>}</section>}
